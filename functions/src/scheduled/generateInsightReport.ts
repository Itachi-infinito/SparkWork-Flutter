...
// Miroir de lib/core/constants/app_sectors.dart — tenir synchronisé si un
// secteur est ajouté/renommé côté app.
const SECTORS = ['horeca', 'digital', 'rh', 'droit', 'commerce', 'btp', 'sante', 'logistique'];
const DEFAULT_REGION = 'Belgique';

/**
 * Génère un rapport insight marché par secteur le 1er de chaque mois à 4h
 * UTC pour le mois précédent. Agrège les données de la plateforme (salaires,
 * compétences recherchées, temps moyen de match) pour donner aux recruteurs
 * Pro une vue propre à leur(s) secteur(s).
 */
export const generateInsightReport = onSchedule(
  ...
  async () => {
    ...
    for (const sector of SECTORS) {
      await generateReportForSector(db, sector, month, year, monthStart, monthEnd, now);
    }
  }
);

async function generateReportForSector(
  ...
  const reportId = `${year}-${String(month).padStart(2, '0')}-${sector}`;
  ...
  // ─── Offres du secteur publiées ce mois-ci ─────────────────────────────
  ...
  const offerIds = new Set(offersSnap.docs.map((d) => d.id));

  // ─── Profils candidats actifs sur ce secteur ───────────────────────────
  ...
  // ─── Compétences les plus recherchées (dans les offres du secteur) ────
  ...
  // ─── Temps moyen de match sur les offres du secteur ────────────────────
  ...
  for (const doc of matchesSnap.docs) {
    const data = doc.data();
    if (!offerIds.has(data.jobOfferId as string)) continue;
  ...
  // Un secteur sans aucune activité ce mois-ci ne mérite pas de rapport vide.
  if (totalActiveProfiles === 0 && offersSnap.size === 0) {
    logger.info(`generateInsightReport: skipping ${reportId} — no activity`);
    return;
  }
  ...
  logger.info(`generateInsightReport: generated ${reportId} — ${totalActiveProfiles} profiles, ${offersSnap.size} offers, ${matchCount} matches`);
}
